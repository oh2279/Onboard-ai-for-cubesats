...
# Irish 데이터셋 경로 설정
base_dir = "/home/gpuadmin/data/satellite_data/95_cloud"

# l7_irish_update 학습 데이터셋 구성
...
# 추가로 사용할 stylized 데이터셋 경로 설정
stylized_source_dir = "/home/gpuadmin/satellite/minsu/StyleID/stylized_95/source"
stylized_label_dir  = "/home/gpuadmin/satellite/minsu/StyleID/stylized_95/label"
stylized_image_files = sorted(glob.glob(os.path.join(stylized_source_dir, "*.png")))
stylized_image_names = [os.path.basename(p) for p in stylized_image_files]

stylized_dataset = CloudDataset(
    img_files=stylized_image_names,
    images_dir=stylized_source_dir,
    masks_dir=stylized_label_dir,
    transform_image=transform_image,
    transform_mask=transform_mask
)

# 두 데이터셋 결합
combined_train_dataset = ConcatDataset([train_dataset, stylized_dataset])
...
# 가중치 파일 이름 변수 설정
weight_filename = "best_scratch_stylized_95cloud_pretrained_lraspp_mobilenetv3.pth"
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val':
                ...
                if miou > best_miou:
                    ...
                    print(f"Best {weight_filename} saved.")
            else:
